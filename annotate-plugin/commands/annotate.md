...
(if none is open, ask which URL); register and show the overlay once via
`addInitScript` (per the skill); then tell them it's ready and stop. You do NOT
need to block or poll — the annotate channel pushes a `<channel source="annotate">`
event into the session when they hit Send.
